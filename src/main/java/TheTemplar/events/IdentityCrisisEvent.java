...
import static TheTemplar.TemplarMod.makeEventPath;

public class IdentityCrisisEvent extends AbstractImageEvent {
    private static final float HEALTH_LOSS_PERCENTAGE = 0.03F; // 3%
    private static final float HEALTH_LOSS_PERCENTAGE_LOW_ASCENSION = 0.05F; // 5%

    public static final String ID = TemplarMod.makeID("IdentityCrisisEvent");
            ...
            case 0: // While you are on screen number 0 (The starting screen)
                switch (i) {
                    case 0: // If you press button the first button (Button at index 0), in this case: Inspiration.
                        ...
                        AbstractRelic relicToAdd = RelicLibrary.starterList.get(AbstractDungeon.relicRng.random(RelicLibrary.starterList.size() - 1))
                                                                           .makeCopy();
                        // Get a random starting relic

                        AbstractDungeon.getCurrRoom()
                                       .spawnRelicAndObtain((float) (Settings.WIDTH / 2), (float) (Settings.HEIGHT / 2), relicToAdd);
